...
Imports System.IO
Public Class Frm_ReceiptUpdate
    Dim SSQL As String
    ...
    Private Sub Frm_ReceiptUpdate_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            If Trim(Txt_BookingNo.Text) <> "" Then
                ...
                If gdataset.Tables("HallHdr").Rows.Count > 0 Then
                    ...
                    sqlstring = "SELECT VoucherNo,VoucherDate,Amount,Description,ISNULL(Ref_No,'') AS Ref_No FROM JOURNALENTRY WHERE SLCode = '" & Trim(gdataset.Tables("HallHdr").Rows(0).Item("MCODE")) & "' AND AccountCode = '" & Trim(PartyAcccode) & "' AND CreditDebit = 'CREDIT' AND ISNULL(VOID,'') <> 'Y' AND ISNULL(Ref_No,'') = '" & Txt_BookingNo.Text & "' "
                    sqlstring = sqlstring & " UNION ALL"
                    sqlstring = sqlstring & " SELECT VoucherNo,VoucherDate,Amount,Description,ISNULL(Ref_No,'') AS Ref_No FROM JOURNALENTRY WHERE SLCode = '" & Trim(gdataset.Tables("HallHdr").Rows(0).Item("MCODE")) & "' AND AccountCode = '" & Trim(PartyAcccode) & "' AND CreditDebit = 'CREDIT' AND ISNULL(VOID,'') <> 'Y' AND ISNULL(Ref_No,'') = '' ORDER BY 1 "
                    GCONNECTION.getDataSet(sqlstring, "PARTY_OTHERENTRY")
                    If gdataset.Tables("PARTY_OTHERENTRY").Rows.Count > 0 Then
                        For I = 0 To gdataset.Tables("PARTY_OTHERENTRY").Rows.Count - 1
                            With sSGrid
                                .Row = I + 1
                                .Col = 1
                                .Text = gdataset.Tables("PARTY_OTHERENTRY").Rows(I).Item("VoucherNo")
                                .Col = 2
                                .Text = Format(gdataset.Tables("PARTY_OTHERENTRY").Rows(I).Item("VoucherDate"), "dd/MM/yyyy")
                                .Col = 3
                                .Text = gdataset.Tables("PARTY_OTHERENTRY").Rows(I).Item("Amount")
                                .Col = 4
                                .Text = gdataset.Tables("PARTY_OTHERENTRY").Rows(I).Item("Description")
                                .Col = 5
                                If gdataset.Tables("PARTY_OTHERENTRY").Rows(I).Item("Ref_No") = Trim(Txt_BookingNo.Text) Then
                                    .Text = "Yes"
                                Else
                                    .Text = "No"
                                End If
    ...
    Public Sub checkValidation()
        ...
        Try
            ...
            If sSGrid.DataRowCnt = 0 Then
                MessageBox.Show("No Receipt to Update ", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Txt_BookingNo.Focus()
                Exit Sub
            End If
            boolchk = True
    ...
    Private Sub CmdAdd_Click(sender As Object, e As EventArgs) Handles CmdAdd.Click
        Dim strSQL, SQL, ChkStatus, Voucherno As String
            ...
            If CmdAdd.Text = "Update[F7]" Then
                With sSGrid
                    For i = 1 To .DataRowCnt
                        ...
                        .Col = 5
                        ChkStatus = Trim(.Text)
                        If ChkStatus = "Yes" Then
                            .Row = i
                            .Col = 1
                            Voucherno = Trim(.Text)
                            strSQL = "UPDATE JOURNALENTRY SET Ref_No = '" & Trim(Txt_BookingNo.Text) & "' WHERE VoucherNo = '" & Trim(Voucherno) & "'"
                        ...
                        Else
                            ...
                            .Col = 1
                            Voucherno = Trim(.Text)
                            strSQL = "UPDATE JOURNALENTRY SET Ref_No = '' WHERE VoucherNo = '" & Trim(Voucherno) & "'"
                            ReDim Preserve INSERT(INSERT.Length)
                            INSERT(INSERT.Length - 1) = strSQL
                        End If
                    Next
